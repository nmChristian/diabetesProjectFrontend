...
})
{
    ...
    const offsets = sizes.map<number>(d => (d == 0) ? 0 : offset)
    // startPos = previousPos + previousHeight + previousOffset
    const startPos : number[] = sizes.reduce<number[]>((startPos, _, i) => startPos.concat(i == 0 ? yScale(0) :  startPos[i - 1] - heights[i - 1] - offsets[i - 1]), [])
